...
@NullMarked
public class IsEqualsOperator implements RSQLCustomOperator<String> {

	private final ComparisonOperator comparisonOperator = new ComparisonOperator("==", Arity.nary(1));
	...
	public Predicate toPredicate(RSQLCustomPredicateInput input) {
		...
		return cb.equal(input.getPath(), input.getArguments().get(0));
	}
}
