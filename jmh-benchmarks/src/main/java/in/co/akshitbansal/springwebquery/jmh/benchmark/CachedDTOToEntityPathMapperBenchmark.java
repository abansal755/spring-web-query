...
public class CachedDTOToEntityPathMapperBenchmark {
	...
	@State(Scope.Thread)
	public static class CacheMissMapperState {

		public CachedDTOToEntityPathMapper pathMapper;

		@Setup(Level.Invocation)
		public void setup() {
			DTOToEntityPathMapperFactory factory = new DTOToEntityPathMapperFactory(10, 32);
			pathMapper = (CachedDTOToEntityPathMapper) factory.newMapper(UserEntity.class, User.class);
		}
	}

	@State(Scope.Thread)
	public static class CacheHitMapperState {
		...
		public void setup() {
			...
			// populate cache
			pathMapper.map("userId");
			pathMapper.map("profile.primaryAddress.city");
			pathMapper.map("accounts.portfolios.positions.lots.serialNumber");
			pathMapper.map("accounts.portfolios.positions.security.issuer.compliance.marketRegion");
		}
	}
